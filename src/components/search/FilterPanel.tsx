...
export function FilterPanel({ filters, setFilters, onClose, isMobile }: FilterPanelProps) {
  ...
  const FilterSection = ({ title, category, options }: { title: string, category: keyof SearchFilters, options: string[] }) => (
    <div className="mb-6">
    ...
    <div className={`flex flex-col h-full bg-white ${!isMobile ? 'border border-law-border rounded-2xl p-5 shadow-sm' : ''}`}>
      ...
      <div className={`flex-1 overflow-y-auto ${isMobile ? 'p-5' : ''}`}>
        ...
        <FilterSection 
          title="Legal Domain" 
          category="domain" 
          options={['Criminal Law', 'Civil Law', 'Constitutional Law', 'Corporate Law', 'Cyber Law']} 
        />
        ...
        <FilterSection 
          title="State" 
          category="state" 
          options={['Union of India', 'Maharashtra', 'Delhi', 'Karnataka']} 
        />
      </div>
